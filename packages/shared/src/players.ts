...
export type PackResult = { cards: LootCard[]; currency: number };
export type CaseResult = { prize: LootCard; reel: LootCard[]; landing: number; currency: number };
export type LadderRow = { username: string; elo: number };
export type MatchRewards = { elo: number; currency: number; gained: number };
