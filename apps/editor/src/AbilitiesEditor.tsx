...
export function AbilitiesEditor({ card, cards, onChange }: Props) {
  ...
  return <fieldset><legend>{t('abilities')}</legend><div className="trigger-buttons">{triggers.map(trigger => <button key={trigger} type="button" disabled={card.abilities.length >= 8} onClick={() => add(trigger)}>+ {t(trigger)}</button>)}</div>
    {card.abilities.map((a, index) => <fieldset key={index} className="ability-editor"><legend>{a.name || t(a.trigger)}</legend>
      <label>{ru ? 'Название способности' : 'Ability name'}<input value={a.name ?? ''} placeholder={t(a.trigger)} maxLength={100} onChange={e => update(index, { name: e.target.value })} /></label>
      ...
      {a.effectId === 'summon' ? <label>{ru ? 'Карта для призыва' : 'Card to summon'}<select value={String(a.params.cardId)} onChange={e => update(index, { params: { ...a.params, cardId: e.target.value } })}>{[card, ...cards.filter(c => c.id !== card.id)].map(c => <option key={c.id} value={c.id}>{c.name[i18n.language] || c.name.ru}</option>)}</select></label> : <label>{t('target')}<select disabled={a.trigger === 'enrage'} value={String(a.params.target)} onChange={e => update(index, { params: { ...a.params, target: e.target.value } })}>{['self','allEnemies',...(['damage','heal'].includes(a.effectId) ? ['enemyHero'] : [])].map(k => <option key={k} value={k}>{t(k)}</option>)}</select></label>}
      <label>{a.effectId === 'summon' ? (ru ? 'Количество существ' : 'Minion count') : t('amount')}<input type="number" min={1} max={a.effectId === 'summon' ? 7 : 20} value={Number(a.params.amount)} onChange={e => update(index, { params: { ...a.params, amount: Number(e.target.value) } })} /></label>
      {a.trigger === 'enrage' && <p>{ru ? 'Срабатывает при переходе из полного здоровья в раненое состояние. После полного лечения может сработать снова.' : 'Triggers when the minion goes from full health to wounded. It can trigger again after a full heal.'}</p>}
      <button onClick={() => onChange({ ...card, abilities: card.abilities.filter((_, n) => n !== index) })}>{t('remove')}</button>
    </fieldset>)}<p>{ru ? 'Призыв ограничен 7 существами на вашей стороне. Боевой клич срабатывает только при розыгрыше из руки.' : 'Summons are capped at 7 minions on your side. Battlecry only triggers when played from hand.'}</p></fieldset>;
}
