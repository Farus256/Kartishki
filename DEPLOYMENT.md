...
Редактор пишет в локальный сервер: `apps/server/data/catalog.json`, плюс файлы в `portraits/` и `music/`. Render читает **только** этот каталог из git. Эфемерный диск после деплоя не хранит публикации редактора.

Клиентский бандл каталог не содержит — карты и музыка приходят с `/api/catalog`, `/api/portraits`, `/api/music`.

1. Править карты в локальном редакторе (http://127.0.0.1:5174).
2. Сохранить / опубликовать.
3. Проверка без git:
...
4. Выкладка (валидация, тесты каталога, сборка сервера, commit только файлов каталога, push текущей ветки):

```powershell
npm run deploy:cards
```

Черновик без commit/push:

```powershell
npm run deploy:cards -- --dry-run
```

5. Дождаться деплоя Render по пушу в GitHub.
6. Проверить прод: `GET https://<service>.onrender.com/api/catalog`.

Скрипт не делает `git add .`, не коммитит `.env`, `PNG/`, `Spritesheets/`, `Vector/`, `node_modules`, `dist`. Не force-push, не меняет ветку. Если в индексе уже лежат чужие файлы — откажется.

Сборка клиента в этой команде не нужна. Cloudflare Pages при пуше ветки может пересобраться сам; содержимое JS от каталога не зависит.
